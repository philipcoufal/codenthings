...
$savepath="D:\PVCS\Scripts_copy from here\Scripts\DOCTemplate\Performance Testing Report v0.2.docx"
...
# changed do to the limited jason file size of 2 MB
...
$filedata = [System.IO.File]::ReadAllText("D:\PVCS\Scripts_copy from here\Scripts\Report\ReportExport.JSON") #Using default encoding
$report = $json.Deserialize($filedata, [System.Object])
$filedata = $null
$json = $null
...
Function CreateChart($model, $counter, $cdata) {
    ...
    for ($t=0; $t -lt $cdata.length; $t++) {
        ...
        $cname = "\\*\"+$counter.name
        if (($counter.name -like "*Disk*") -or ($counter.name -like "*Network*")) { $cname = "\\*\"+$counter.name.Replace("*", $cdata[$t].instance) }
...
foreach ($model in $report.models) {    # foreach ($model in $models) { 
    write-host "---" $model.name -foreground "green"
    ...
    foreach ($counter in $model.counters | Where-Object {$_.name -like "*% Processor Time*" -or $_.name -like "*Available MBytes*" -or $_.name -like "*% Idle Time*" -or $_.name -like "*Bytes Total*"}) {   ##foreach ($counter in $counters) {
        if ($counter.values) {
            Write-Host "--" $counter.name -foreground "green"
            ...
            foreach ($row in $counter.values) {    ##foreach ($row in $counter.group) {
                ...
                $rowTitle = $row.server
                if (($counter.name -like "*Disk*") -or ($counter.name -like "*Network*") -or ($counter.name -like "*Process(*")) { $rowTitle += "(" +$row.instance+ ")" }
                $rowTitle += " (" +$desc+ ") "
                ...
                if (($counter.name -like "*Context Switches*") -or ($counter.name -like "*Processor Queue*")) { $tol = [int]::Parse($tol) * [int]::Parse($serverInfo.cpus) }
                write-host $tol
            ...
            foreach ($rchart in $counter.values | Group-Object {$_.chartnum}) {
                write-host "Adding chart..." -foreground "Green"
...
foreach ($model in $report.models) {   ##foreach ($model in $models) {
    ...
    foreach ($counter in $model.counters) {   ##foreach ($counter in $counters) {
        ...
        if ($tdata) {
            ...
            Write-Host "Creating table..." -foreground "green"
            $rows = $tdata.length + 1
            $cols = 4
            $range = $selection.Range
            $table = $doc.Tables.add($range,$rows,$cols)
            ...
            $table = $selection.EndKey(6)
            write-host "Table complete!" -foreground "green"
            ##$selection.TypeParagraph()
            $selection.Style="Caption"
            $selection.TypeText($model.name+ " - " +$counter.name+ " (" +$counterInfo.tol.desc+ ")")
            ##$selection.TypeParagraph()
            ...
            foreach ($server in $tdata | Group-Object {$_.server}) {
                ...
                ##if ($c % 4 -eq 0) {
                    ...
                    write-host "Saving chart..." -foreground "green"
                    ##Start-Sleep -s 2
